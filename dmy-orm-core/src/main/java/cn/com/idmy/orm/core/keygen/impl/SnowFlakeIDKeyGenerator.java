...
public class SnowFlakeIDKeyGenerator implements IKeyGenerator {
    ...
    protected long getDataCenterId(long maxDataCenterId) {
        ...
        try {
            ...
            throw OrmExceptions.wrap(e, "dataCenterId: %s", e.getMessage());
        }
        return id;
    }
        ...
        if (currentTimeMillis < lastTimeMillis) {
            ...
            if (offset <= offsetPeriod) {
                try {
                    ...
                    if (currentTimeMillis < lastTimeMillis) {
                        throw OrmExceptions.wrap("Clock moved backwards, please check the time. Current timestamp: %d, last used timestamp: %d", currentTimeMillis, lastTimeMillis);
                    }
