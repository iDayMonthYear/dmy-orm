package cn.com.idmy.orm.core.dialect.impl;


import cn.com.idmy.orm.core.dialect.Dialect;
import cn.com.idmy.orm.core.dialect.KeywordWrap;
import cn.com.idmy.orm.core.dialect.LimitOffsetProcessor;
...
public class DefaultDialectImpl implements Dialect {
    ...
    public DefaultDialectImpl(LimitOffsetProcessor limitOffsetProcessor) {
        this.limitOffsetProcessor = limitOffsetProcessor;
    }

    public DefaultDialectImpl(KeywordWrap keywordWrap, LimitOffsetProcessor limitOffsetProcessor) {
    ...
    public String forHint(String hintString) {
        return StrUtil.isNotBlank(hintString) ? HINT_START + hintString + HINT_END : EMPTY;
    }
    ...
    public String forUpdateByQuery(QueryWrapper queryWrapper, Row row) {
        ...
        if (queryTables == null || queryTables.size() != 1) {
            throw OrmExceptions.wrap(LocalizedFormats.UPDATE_ONLY_SUPPORT_1_TABLE);
        }
    ...
    public String buildSelectSql(QueryWrapper queryWrapper) {
        ...
        if (allTables.size() > 1 && selectColumns != null && selectColumns.size() > 1) {
            IntStream.range(0, selectColumns.size())
                    ...
                    .stream()
                    // 需要处理别名的情况
                    .filter(e -> StrUtil.isNotBlank(e.getValue().getName()))
                    .filter(e -> StrUtil.isBlank(e.getValue().getAlias()))
                    ...
                    .filter(e -> StrUtil.isNotBlank(e.getValue().getTable().getName()))
                    // 添加别名并放回原集合索引位置
    ...
    public String buildDeleteSql(QueryWrapper queryWrapper) {
        ...
        if (joinTables != null && !joinTables.isEmpty()) {
            ...
            if (StrUtil.isNotBlank(queryTable.getSchema())) {
    ...
    public String forDeleteEntityById(TableInfo tableInfo) {
        ...
        //正常删除
        if (StrUtil.isBlank(logicDeleteColumn)) {
    ...
    public String forUpdateEntity(TableInfo tableInfo, Object entity, boolean ignoreNulls) {
        ...
        if (StrUtil.isNotBlank(versionColumn)) {
    ...
    public String forSelectEntityListByIds(TableInfo tableInfo, Object[] primaryValues) {
        ...
        String logicDeleteColumn = tableInfo.getLogicDeleteColumnOrSkip();
        Object[] tenantIdArgs = tableInfo.buildTenantIdArgs();
        if (StrUtil.isNotBlank(logicDeleteColumn) || ArrayUtil.isNotEmpty(tenantIdArgs)) {
            sql.append(BRACKET_LEFT);
        }
    ...
    protected void buildWhereSql(StringBuilder sqlBuilder, QueryWrapper queryWrapper, List<QueryTable> queryTables, boolean allowNoCondition) {
        ...
        if (whereQueryCondition != null) {
            ...
            if (StrUtil.isNotBlank(whereSql)) {
                ...
                throw OrmExceptions.wrap(LocalizedFormats.UPDATE_OR_DELETE_NOT_ALLOW);
            }
        } else {
            if (!allowNoCondition) {
                throw OrmExceptions.wrap(LocalizedFormats.UPDATE_OR_DELETE_NOT_ALLOW);
